...
async fn main() -> Result<()> {
    ...
    let repo = match GitRepo::open(&path) {
        ...
        Err(_) => {
            ...
            println!();
            println!("{}", "Tip: Run 'git init' to initialize a Git repository".yellow());
            return Ok(());
        }
    };
...
fn handle_status_command(repo: GitRepo, verbose: bool) -> Result<()> {
    ...
    if status.is_clean {
        ...
        println!("{}: {}", "Total uncommitted changes".bold(), 
                 status.total_changes().to_string().yellow());
        
        if verbose {
            println!();
            println!("{}", "Tip: Use 'git add .' to stage all changes".cyan());
            println!("{}", "     Use 'git commit -m \"message\"' to commit staged changes".cyan());
            println!("{}", "     Use 'rust-commit commit' to generate AI commit message".cyan());
        }
    }
    ...
    if let Some(name) = branch_info.name {
        if name == "unborn" {
            println!("{}: {}", "Branch".yellow().bold(), "No commits yet (unborn branch)");
        } else {
            ...
            if let Some(tracking) = branch_info.tracking_info {
                ...
                if tracking.ahead > 0 || tracking.behind > 0 {
                    ...
                    if tracking.behind > 0 {
                        status_parts.push(format!("{} behind", tracking.behind).yellow().to_string());
                    }
...
async fn handle_commit_command(
    ...
    let api_key = api_key
        ...
        .or_else(|| {
            CommitUI::get_api_key(&config.ai.provider).ok()
        })
