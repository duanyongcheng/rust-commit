...
- 🐛 **调试模式** - 内置调试功能，方便排查问题
- ✅ **智能暂存** - 自动检测未暂存更改并提示用户确认
- 🔐 **安全优先** - 支持环境变量存储 API 密钥，避免明文存储
...
2. **避免：直接在配置文件中存储 API 密钥**
   - 如果必须使用配置文件，确保文件权限设置为 600
   - 永远不要将包含 API 密钥的配置文件提交到版本控制
...
4. **提交失败**
   - 确保 Git 用户已配置（`git config user.name` 和 `user.email`）
   - 检查是否有文件权限问题
   - 查看具体的 Git 错误信息
